...
export default function DueListPage() {
  ...
  const fetchPartyDues = async () => {
    try {
      const response = await axios.get('http://127.0.0.1:8000/api/brokerage/party-dues/');
  ...
  const handleSelectParty = async (partyId: number) => {
    ...
    try {
      const response = await axios.get(`http://127.0.0.1:8000/api/brokerage/party-dues/${partyId}/`);
